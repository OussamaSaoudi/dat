...
private def txnTable(w: WorkloadContext, numInserts: Int): Unit = {
...
workload("txn_basic", "Basic SetTransaction tracking", "txn") { w =>
  txnTable(w, 2)
  val t = w.table("tbl")
...
workload("txn_update", "SetTransaction version update", "txn") { w =>
  txnTable(w, 3)
  val t = w.table("tbl")
  w.mutateTable(t) { tableDir =>
    val v1 = tableDir.resolve("_delta_log/00000000000000000001.json")
...
workload("txn_duplicate_appid", "Two SetTransaction for same appId in separate commits", "txn") { w =>
  txnTable(w, 3)
  val t = w.table("tbl")
  w.mutateTable(t) { tableDir =>
    val v1 = tableDir.resolve("_delta_log/00000000000000000001.json")
...
workload("txn_multiple_apps_1", "Multiple app transactions - app-1", "txn") { w =>
  txnTable(w, 3)
  val t = w.table("tbl")
  w.mutateTable(t) { tableDir =>
    val v1 = tableDir.resolve("_delta_log/00000000000000000001.json")
...
workload("txn_multiple_apps_2", "Multiple app transactions - app-2", "txn") { w =>
  txnTable(w, 3)
  val t = w.table("tbl")
  w.mutateTable(t) { tableDir =>
    val v1 = tableDir.resolve("_delta_log/00000000000000000001.json")
...
workload("txn_batch_app1", "Multiple SetTransactions in single commit - app-1", "txn") { w =>
  txnTable(w, 2)
  val t = w.table("tbl")
  w.mutateTable(t) { tableDir =>
    val v1 = tableDir.resolve("_delta_log/00000000000000000001.json")
...
workload("txn_batch_app3", "Multiple SetTransactions in single commit - app-3", "txn") { w =>
  txnTable(w, 2)
  val t = w.table("tbl")
  w.mutateTable(t) { tableDir =>
    val v1 = tableDir.resolve("_delta_log/00000000000000000001.json")
...
workload("txn_with_last_updated", "SetTransaction with lastUpdated", "txn") { w =>
  txnTable(w, 2)
  val t = w.table("tbl")
  w.mutateTable(t) { tableDir =>
    val v1 = tableDir.resolve("_delta_log/00000000000000000001.json")
...
workload("txn_without_last_updated", "SetTransaction without lastUpdated", "txn") { w =>
  txnTable(w, 2)
  val t = w.table("tbl")
  w.mutateTable(t) { tableDir =>
    val v1 = tableDir.resolve("_delta_log/00000000000000000001.json")
...
workload("txn_after_checkpoint", "SetTransaction survives checkpoint", "txn") { w =>
  txnTable(w, 3)
  val t = w.table("tbl")
  w.mutateTable(t) { tableDir =>
    val v1 = tableDir.resolve("_delta_log/00000000000000000001.json")
...
workload("txn_at_version_1", "SetTransaction at version 1", "txn") { w =>
  txnTable(w, 4)
  val t = w.table("tbl")
  w.mutateTable(t) { tableDir =>
    val v1 = tableDir.resolve("_delta_log/00000000000000000001.json")
...
workload("txn_future_version", "SetTransaction version greater than commit version", "txn") { w =>
  txnTable(w, 2)
  val t = w.table("tbl")
  w.mutateTable(t) { tableDir =>
    val v1 = tableDir.resolve("_delta_log/00000000000000000001.json")
