...
workload("cc_001_create_with_constraint", "CREATE TABLE with valid check constraint", "checkConstraints") { w =>
...
workload("cc_002_show_tblproperties", "See constraints in table properties", "checkConstraints") { w =>
...
workload("cc_003_delta_history", "Delta history for constraints", "checkConstraints") { w =>
...
workload("cc_004_case_insensitive_drop", "Drop constraint is case insensitive", "checkConstraints") { w =>
  w.sql("""CREATE TABLE tbl (x INT) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true')""")
  w.sql("INSERT INTO tbl VALUES (1),(2),(3)")
  w.sql("ALTER TABLE tbl ADD CONSTRAINT MyConstraint CHECK (x > 0)")
...
workload("cc_005_varchar_constraint", "Constraint induced by varchar", "checkConstraints") { w =>
...
workload("cc_006_basic_constraint", "Read table with check constraint", "checkConstraints") { w =>
  w.sql("""CREATE TABLE tbl (id INT, value STRING) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true')""")
  w.sql("ALTER TABLE tbl ADD CONSTRAINT positive_id CHECK (id > 0)")
...
workload("cc_007_multiple_constraints", "Read table with multiple check constraints", "checkConstraints") { w =>
...
workload("cc_008_nested_constraint", "Read with nested column constraint", "checkConstraints") { w =>
...
workload("cc_009_array_constraint", "Read with array size constraint", "checkConstraints") { w =>
...
workload("cc_010_length_constraint", "Read with string length constraint", "checkConstraints") { w =>
...
workload("cc_011_compound_constraint", "Read with compound constraint", "checkConstraints") { w =>
...
workload("cc_012_not_null_constraint", "Read with NOT NULL-like constraint", "checkConstraints") { w =>
...
workload("cc_013_time_travel", "Time travel with constraints", "checkConstraints") { w =>
...
workload("cc_014_time_type_constraint", "CHECK constraints with TIME type columns", "checkConstraints") { w =>
...
workload("cc_015_time_multiple_conditions", "CHECK constraints with TIME type - multiple conditions", "checkConstraints") { w =>
...
workload("cc_016_allowed_expressions", "Creating constraints with allowed expressions", "checkConstraints") { w =>
...
workload("cc_017_column_mapping", "Read constraints with column mapping", "checkConstraints", "columnMapping") { w =>
  w.sql("""CREATE TABLE tbl (id INT, value STRING) USING delta
...
workload("cc_018_drop_feature", "Drop constraint before drop feature", "checkConstraints") { w =>
  w.sql("""CREATE TABLE tbl (x INT) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true')""")
  w.sql("INSERT INTO tbl VALUES (1),(2),(3)")
  w.sql("ALTER TABLE tbl ADD CONSTRAINT positive CHECK (x > 0)")
...
workload("cc_019_boolean_column_names", "Boolean column with constraints", "checkConstraints") { w =>
...
workload("cc_020_decimal_constraint", "Constraint with decimal column", "checkConstraints") { w =>
...
workload("cc_complex_expr", "Check constraint with AND/OR expression", "checkConstraints") { w =>
...
workload("cc_null_aware", "IS NOT NULL constraint on nested struct", "checkConstraints") { w =>
  w.sql("""CREATE TABLE tbl (id INT, info STRUCT<name: STRING, age: INT>) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true')""")
