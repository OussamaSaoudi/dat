/**
 * Data skipping advanced workloads (ds_* family).
 ...
 * generated columns, DVs, partitioned skipping.
 *
 * Run: ./bin/generate-workload.sh tables/data_skipping.scala
 */
...
workload("ds_top_level_single_1", "top level, single 1", "dataSkipping") { w =>
...
workload("ds_nested_single_1", "nested, single 1", "dataSkipping") { w =>
...
workload("ds_double_nested_single_1", "double nested, single 1", "dataSkipping") { w =>
...
workload("ds_nested_struct_predicate", "Nested struct field predicate", "dataSkipping") { w =>
...
workload("ds_complex_nested", "Complex nested predicates", "dataSkipping") { w =>
...
workload("ds_and_simple", "and statements - simple", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a LONG) USING delta")
  w.sql("INSERT INTO tbl VALUES (1), (2)")
  val t = w.table("tbl")
...
workload("ds_and_two_fields", "and statements - two fields", "dataSkipping") { w =>
...
workload("ds_and_one_side_unsupported", "AND one side unsupported", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a LONG) USING delta")
  w.sql("INSERT INTO tbl VALUES (1), (2)")
  val t = w.table("tbl")
...
workload("ds_or_simple", "or statements - simple", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a LONG) USING delta")
  w.sql("INSERT INTO tbl VALUES (1), (2)")
  val t = w.table("tbl")
...
workload("ds_or_two_fields", "or statements - two fields", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a LONG, b LONG) USING delta")
  w.sql("INSERT INTO tbl VALUES (1, 10), (2, 20)")
  val t = w.table("tbl")
...
workload("ds_or_one_side_unsupported", "OR one side unsupported", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a LONG) USING delta")
  w.sql("INSERT INTO tbl VALUES (1), (2)")
  val t = w.table("tbl")
...
workload("ds_not_simple", "not statements - simple", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a LONG) USING delta")
  w.sql("INSERT INTO tbl VALUES (1), (2)")
  val t = w.table("tbl")
...
workload("ds_not_and", "NOT with AND (De Morgan)", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a LONG) USING delta")
  w.sql("INSERT INTO tbl VALUES (1), (2)")
  val t = w.table("tbl")
...
workload("ds_not_or", "NOT with OR (De Morgan)", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a LONG) USING delta")
  w.sql("INSERT INTO tbl VALUES (1), (2)")
  val t = w.table("tbl")
...
workload("ds_starts_with", "starts with", "dataSkipping") { w =>
...
workload("ds_starts_with_nested", "LIKE on nested string fields", "dataSkipping") { w =>
...
workload("ds_string_patterns", "String comparisons and LIKE", "dataSkipping") { w =>
...
workload("ds_long_strings_min", "long strings, long min", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a STRING) USING delta")
...
workload("ds_long_strings_max", "long strings, long max", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a STRING) USING delta")
  val longZ = "z" * 32 + "a"
  val longY = "z" * 32 + "b"
...
workload("ds_in_set", "IN set predicates", "dataSkipping") { w =>
...
workload("ds_in_list", "IN list predicate", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a INT) USING delta")
...
workload("ds_in_nested", "IN on nested field", "dataSkipping") { w =>
...
workload("ds_in_with_nulls_mixed", "IN list containing NULL", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a INT) USING delta")
...
workload("ds_in_with_nulls_only", "IN list with only nulls in file", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a INT) USING delta")
...
workload("ds_in_with_thresholds", "IN with varying size", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a INT) USING delta")
...
workload("ds_not_in", "NOT IN predicate", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a INT) USING delta")
  w.sql("INSERT INTO tbl VALUES (1), (2), (3)")
  val t = w.table("tbl")
...
workload("ds_is_null", "IS NULL predicate pushdown", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a INT) USING delta")
  w.sql("INSERT INTO tbl VALUES (1), (NULL), (3)")
  val t = w.table("tbl")
...
workload("ds_is_not_null", "IS NOT NULL predicate pushdown", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a INT) USING delta")
  w.sql("INSERT INTO tbl VALUES (1), (NULL), (3)")
  val t = w.table("tbl")
...
workload("ds_isnull_complex_expr", "IS NULL with complex expressions", "dataSkipping") { w =>
...
workload("ds_nulls_only_null", "nulls - only null in file", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a LONG) USING delta")
...
workload("ds_nulls_only_nonnull", "only non-null in file", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a LONG) USING delta")
...
workload("ds_nulls_mixed", "nulls - null + not-null in same file", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a LONG) USING delta")
  w.sql("INSERT INTO tbl VALUES (1), (NULL), (3)")
  val t = w.table("tbl")
  w.read(t, predicate = "a IS NULL")
...
workload("ds_nulls_nonnulls_only", "non-nulls only in file", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a LONG) USING delta")
  w.sql("INSERT INTO tbl VALUES (1), (2), (3)")
  val t = w.table("tbl")
  w.read(t, predicate = "a IS NULL")
...
workload("ds_nulls_partial_stats", "non-nulls only, partial stats", "dataSkipping") { w =>
...
workload("ds_null_safe_eq", "Null-safe equals operator (<=>)", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a INT) USING delta")
  w.sql("INSERT INTO tbl VALUES (1), (NULL), (3)")
  val t = w.table("tbl")
  w.read(t, predicate = "a <=> 1")
...
workload("ds_null_string_partition", "Null string partition values", "dataSkipping") { w =>
...
workload("ds_null_mixed_partitions", "Mix of null and non-null across partitions",
    "dataSkipping") { w =>
...
workload("ds_between", "BETWEEN predicate for range queries", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a INT) USING delta")
...
workload("ds_boolean", "boolean comparisons", "dataSkipping") { w =>
...
workload("ds_boolean_column", "Boolean column skipping", "dataSkipping") { w =>
...
workload("ds_numeric_types", "Numeric type comparisons", "dataSkipping") { w =>
...
workload("ds_tinyint_smallint", "TINYINT and SMALLINT column stats", "dataSkipping") { w =>
...
workload("ds_float_special_values", "FLOAT with NaN, Infinity, -0.0", "dataSkipping") { w =>
...
workload("ds_binary_type", "BINARY column (no min/max, only nullCount)", "dataSkipping") { w =>
...
workload("ds_implicit_cast", "Implicit type cast in predicate", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a LONG) USING delta")
  w.sql("INSERT INTO tbl VALUES (1), (2)")
  val t = w.table("tbl")
...
workload("ds_datetime", "Date and timestamp predicates", "dataSkipping") { w =>
...
workload("ds_timestamp_microsecond", "Microsecond precision timestamp skipping",
    "dataSkipping") { w =>
...
workload("ds_timestamp_ntz_skipping", "NTZ-specific data skipping", "dataSkipping") { w =>
...
workload("ds_year_function", "year() extraction on Date", "dataSkipping") { w =>
...
workload("ds_month_function", "month() with predicates", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (d DATE, value INT) USING delta")
...
workload("ds_trunc_date", "trunc() on Date", "dataSkipping") { w =>
...
workload("ds_date_trunc_timestamp", "date_trunc() on Timestamp", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (ts TIMESTAMP) USING delta")
...
workload("ds_datediff", "datediff() predicate", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (d DATE) USING delta")
...
workload("ds_date_add_sub", "date_add()/date_sub()", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (d DATE) USING delta")
...
workload("ds_multi_file_ranges", "Multiple files with different ranges", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a INT) USING delta")
...
workload("ds_multi_file_time", "TIME type multi-file data skipping", "dataSkipping") { w =>
...
workload("ds_typed_stats", "DECIMAL/DATE/TIMESTAMP/FLOAT/DOUBLE data skipping stats",
    "dataSkipping") { w =>
...
workload("ds_variant_null_stats", "VARIANT NULL/NOT NULL data skipping", "dataSkipping") { w =>
  w.sql("""CREATE TABLE tbl (
...
workload("ds_indexed_names_empty", "empty indexed names disables stats", "dataSkipping") { w =>
  w.sql("""CREATE TABLE tbl (a LONG, b LONG) USING delta
...
workload("ds_indexed_names_subset", "index subset of leaf columns", "dataSkipping") { w =>
...
workload("ds_indexed_names_nested", "naming nested column indexes leaves",
    "dataSkipping") { w =>
  w.sql("""CREATE TABLE tbl (
...
workload("ds_indexed_names_complex", "nested columns with complex types", "dataSkipping") { w =>
  w.sql("""CREATE TABLE tbl (
...
workload("ds_indexed_names_backtick", "backtick escapes", "dataSkipping") { w =>
...
workload("ds_more_cols_than_indexed", "more columns than indexed", "dataSkipping") { w =>
...
workload("ds_missing_stats_cols", "missing stats columns", "dataSkipping") { w =>
  w.sql("""CREATE TABLE tbl (a LONG) USING delta
    TBLPROPERTIES ('delta.dataSkippingNumIndexedCols' = '1')""")
  w.sql("INSERT INTO tbl VALUES (1), (2)")
...
workload("ds_missing_stats_graceful", "Stats missing, read still works (full scan)",
    "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a INT) USING delta")
  w.sql("INSERT INTO tbl VALUES (1), (2), (3)")
  val t = w.table("tbl")
...
workload("ds_stats_config_change", "Stats config changing across versions",
    "dataSkipping") { w =>
...
workload("ds_nested_indexed_0", "nested schema, indexed=0", "dataSkipping") { w =>
...
workload("ds_nested_indexed_3", "nested schema, indexed=3", "dataSkipping") { w =>
  w.sql("""CREATE TABLE tbl (
    a STRUCT<x: LONG, y: LONG>,
    b LONG,
    c STRUCT<p: LONG>
...
workload("ds_nested_indexed_6", "nested schema, indexed=6", "dataSkipping") { w =>
  w.sql("""CREATE TABLE tbl (
...
workload("ds_nested_indexed_9", "nested schema, indexed=9", "dataSkipping") { w =>
  w.sql("""CREATE TABLE tbl (
    a STRUCT<x: LONG, y: LONG, z: LONG>,
...
workload("ds_partitioned", "Partitioned table data skipping", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (id INT, part STRING) USING delta PARTITIONED BY (part)")
...
workload("ds_partition_and_stats", "Partition pruning + stats skipping combined",
    "dataSkipping") { w =>
...
workload("ds_partition_or_predicate", "OR predicate on partition column", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (id INT, part STRING) USING delta PARTITIONED BY (part)")
...
workload("ds_schema_order_mismatch", "Query columns in different order", "dataSkipping") { w =>
...
workload("ds_nonexistent_col_filter", "Filter on non-existent column", "dataSkipping") { w =>
  w.sql("CREATE TABLE tbl (a INT) USING delta")
  w.sql("INSERT INTO tbl VALUES (1), (2)")
  val t = w.table("tbl")
  w.read(t, predicate = "a = 1")
...
workload("ds_generated_col_skipping", "Skipping using generated column stats",
    "dataSkipping") { w =>
  w.sql("""CREATE TABLE tbl (
...
workload("ds_with_dvs_edge", "Data skipping + DVs combined", "dataSkipping", "dv") { w =>
  w.sql("""CREATE TABLE tbl (a INT) USING delta
...
workload("ds_with_dvs_edge_1", "Data skipping + DVs: predicate skips file with DV",
    "dataSkipping", "dv") { w =>
  w.sql("""CREATE TABLE tbl (a INT) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true')""")
  w.sql("INSERT INTO tbl SELECT id FROM range(1, 11)")
...
workload("ds_with_dvs_edge_2", "Data skipping + DVs: entire file deleted via DV",
    "dataSkipping", "dv") { w =>
  w.sql("""CREATE TABLE tbl (a INT) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true')""")
...
workload("ds_stats_col_drop", "Data skipping after column drop (CM=name)",
...
workload("ds_stats_col_rename", "Data skipping after column rename (CM=name)",
    "dataSkipping", "columnMapping") { w =>
...
workload("ds_stats_after_drop", "Stats after column drop with column mapping",
    "dataSkipping", "columnMapping") { w =>
  w.sql("""CREATE TABLE tbl (
    c1 LONG, c2 STRING, c3 FLOAT, c4 DOUBLE,
    c5 TIMESTAMP, c6 TIMESTAMP_NTZ, c7 DATE,
...
workload("ds_stats_after_rename", "Stats after column rename with column mapping",
    "dataSkipping", "columnMapping") { w =>
  w.sql("""CREATE TABLE tbl (
    c1 LONG, c2 STRING, c3 FLOAT, c4 DOUBLE,
    c5 TIMESTAMP, c6 TIMESTAMP_NTZ, c7 DATE,
...
workload("ds_err_001_field_not_found", "FIELD_NOT_FOUND for row commit version filter",
