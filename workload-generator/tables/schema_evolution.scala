...
workload("se_add_col_pred_eq", "Equality predicate on column missing stats in old files", "schema_evolution", "predicate") { w =>
  w.sql("""CREATE TABLE tbl (id INT, name STRING) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true')""")
...
workload("se_add_col_pred_null", "Predicate on null-filled column from old files", "schema_evolution", "predicate") { w =>
  w.sql("""CREATE TABLE tbl (id INT, name STRING) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true')""")
  w.sql("INSERT INTO tbl VALUES (1, 'alice')")
  w.sql("INSERT INTO tbl VALUES (2, 'bob')")
...
workload("se_add_col_read_v1", "Time travel to version before column was added", "schema_evolution", "timeTravel") { w =>
  w.sql("""CREATE TABLE tbl (id INT, name STRING) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true')""")
  w.sql("INSERT INTO tbl VALUES (1, 'alice')")
  w.sql("INSERT INTO tbl VALUES (2, 'bob')")
...
workload("se_add_column_with_default", "Column added then populated with explicit values", "schema_evolution") { w =>
...
workload("se_drop_col_pred", "Data skipping after column drop", "schema_evolution", "column_mapping") { w =>
...
workload("se_drop_col_read_v1", "Time travel to version before column was dropped", "schema_evolution", "column_mapping") { w =>
  w.sql("""CREATE TABLE tbl (id INT, name STRING, value INT) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true',
      'delta.columnMapping.mode' = 'name',
      'delta.minReaderVersion' = '2', 'delta.minWriterVersion' = '5')""")
...
workload("se_drop_readd_same_name", "Drop and re-add column with different type", "schema_evolution", "column_mapping") { w =>
  w.sql("""CREATE TABLE tbl (id INT, x STRING) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true',
      'delta.columnMapping.mode' = 'name',
      'delta.minReaderVersion' = '2', 'delta.minWriterVersion' = '5')""")
...
workload("se_dv_pred_null", "Null-fill predicate + DVs combined", "schema_evolution", "dv") { w =>
  w.sql("""CREATE TABLE tbl (id INT, name STRING) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true')""")
  w.sql("INSERT INTO tbl VALUES (1, 'alice')")
  w.sql("INSERT INTO tbl VALUES (2, 'bob')")
...
workload("se_merge_with_evolution", "MERGE with schema evolution", "schema_evolution") { w =>
  w.sql("""CREATE TABLE target (id INT, name STRING) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true')""")
...
workload("se_nested_field_pred", "Predicate on added nested field", "schema_evolution") { w =>
  w.sql("""CREATE TABLE tbl (id INT, info STRUCT<name: STRING, age: INT>) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true')""")
  w.sql("INSERT INTO tbl VALUES (1, named_struct('name','alice','age',30))")
...
workload("se_nested_field_project", "Nested struct projection with null-fill for old files", "schema_evolution") { w =>
  w.sql("""CREATE TABLE tbl (id INT, info STRUCT<name: STRING, age: INT>) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true')""")
  w.sql("INSERT INTO tbl VALUES (1, named_struct('name','alice','age',30))")
  w.sql("INSERT INTO tbl VALUES (2, named_struct('name','bob','age',25))")
...
workload("se_pred_on_added_col", "IS NOT NULL on added column", "schema_evolution", "predicate") { w =>
  w.sql("""CREATE TABLE tbl (id INT, name STRING) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true')""")
  w.sql("INSERT INTO tbl VALUES (1, 'alice')")
  w.sql("INSERT INTO tbl VALUES (2, 'bob')")
...
workload("se_proj_at_old_version", "Project column at version before schema change", "schema_evolution") { w =>
  w.sql("""CREATE TABLE tbl (id INT, name STRING) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true')""")
  w.sql("INSERT INTO tbl VALUES (1, 'alice')")
  w.sql("INSERT INTO tbl VALUES (2, 'bob')")
...
workload("se_readd_pred", "Predicate on re-added column (new physical name)", "schema_evolution", "column_mapping") { w =>
  w.sql("""CREATE TABLE tbl (id INT, x STRING) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true',
      'delta.columnMapping.mode' = 'name',
      'delta.minReaderVersion' = '2', 'delta.minWriterVersion' = '5')""")
...
workload("se_rename_chain", "Multiple renames a -> b -> c", "schema_evolution", "column_mapping") { w =>
  w.sql("""CREATE TABLE tbl (id INT, a STRING) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true',
      'delta.columnMapping.mode' = 'name',
      'delta.minReaderVersion' = '2', 'delta.minWriterVersion' = '5')""")
...
workload("se_rename_column", "Read after column rename", "schema_evolution", "column_mapping") { w =>
  w.sql("""CREATE TABLE tbl (id INT, name STRING) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true',
      'delta.columnMapping.mode' = 'name',
      'delta.minReaderVersion' = '2', 'delta.minWriterVersion' = '5')""")
...
workload("se_rename_part_pred", "Partition pruning on renamed partition column", "schema_evolution", "column_mapping") { w =>
...
workload("se_rename_partition_column", "Rename partition column", "schema_evolution", "column_mapping") { w =>
  w.sql("""CREATE TABLE tbl (id INT, category STRING, value INT) USING delta
    PARTITIONED BY (category)
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true',
      'delta.columnMapping.mode' = 'name',
...
workload("se_rename_pred", "Predicate on renamed column", "schema_evolution", "column_mapping") { w =>
  w.sql("""CREATE TABLE tbl (id INT, name STRING) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true',
      'delta.columnMapping.mode' = 'name',
      'delta.minReaderVersion' = '2', 'delta.minWriterVersion' = '5')""")
...
workload("se_rename_read_v1", "Version read before rename", "schema_evolution", "column_mapping") { w =>
  w.sql("""CREATE TABLE tbl (id INT, name STRING) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true',
      'delta.columnMapping.mode' = 'name',
      'delta.minReaderVersion' = '2', 'delta.minWriterVersion' = '5')""")
...
workload("se_type_coercion_insert", "INSERT with implicit type cast int to long", "schema_evolution") { w =>
  w.sql("""CREATE TABLE tbl (id LONG, value LONG) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true')""")
  w.sql("INSERT INTO tbl VALUES (1, 100)")
  w.sql("INSERT INTO tbl VALUES (2, 200)")
...
workload("se_add_nested_struct_field", "Add field to nested struct", "schema_evolution") { w =>
  w.sql("""CREATE TABLE tbl (id INT, info STRUCT<name: STRING, age: INT>) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true')""")
  w.sql("INSERT INTO tbl VALUES (1, named_struct('name','alice','age',30))")
  w.sql("INSERT INTO tbl VALUES (2, named_struct('name','bob','age',25))")
...
workload("se_drop_and_readd_same_name", "Drop and re-add column with different type", "schema_evolution", "column_mapping") { w =>
  w.sql("""CREATE TABLE tbl (id INT, x STRING) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true',
      'delta.columnMapping.mode' = 'name',
      'delta.minReaderVersion' = '2', 'delta.minWriterVersion' = '5')""")
...
workload("se_add_top_level_column", "Add top-level column via auto merge", "schema_evolution") { w =>
  w.sql("""CREATE TABLE tbl (id INT, name STRING) USING delta
    TBLPROPERTIES ('delta.enableDeletionVectors' = 'true',
