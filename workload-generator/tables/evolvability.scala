 ...
 *
 * Run: ./bin/generate-workload.sh tables/evolvability.scala
 */
...
workload("ev_batch_read", "Transaction log schema evolvability - batch read",
    "evolvability") { w =>
...
workload("ev_unknown_action_type", "Commit log with unknown action type", "evolvability") { w =>
  w.sql("CREATE TABLE tbl (id LONG) USING delta")
  w.sql("INSERT INTO tbl SELECT id FROM range(10)")
  val t = w.table("tbl")
  w.mutateTable(t) { dir =>
...
workload("ev_protocol", "Protocol evolvability", "evolvability") { w =>
  w.sql("CREATE TABLE tbl (id LONG) USING delta")
...
workload("ev_extra_protocol_fields", "Protocol with extra unknown fields (forward compat)",
    "evolvability") { w =>
  w.sql("CREATE TABLE tbl (id LONG) USING delta")
  w.sql("INSERT INTO tbl SELECT id FROM range(10)")
  val t = w.table("tbl")
...
workload("ev_unknown_protocol_feature", "Protocol with unrecognized writer feature",
    "evolvability") { w =>
  w.sql("CREATE TABLE tbl (id LONG) USING delta")
  w.sql("INSERT INTO tbl SELECT id FROM range(5)")
  val t = w.table("tbl")
...
workload("ev_unknown_writer_feature", "Unknown writer feature does not block read",
    "evolvability") { w =>
  w.sql("CREATE TABLE tbl (id LONG) USING delta")
  w.sql("INSERT INTO tbl SELECT id FROM range(10)")
  val t = w.table("tbl")
...
workload("ev_unknown_reader_feature", "Unknown reader feature blocks read",
...
workload("ev_schema_evolution", "Schema evolution across versions",
...
workload("ev_data_types", "Multiple data types evolvability", "evolvability") { w =>
...
workload("ev_partitioned", "Partitioned table evolvability", "evolvability") { w =>
...
workload("ev_partition_null", "Serialized partition values with null values",
    "evolvability") { w =>
  w.sql("CREATE TABLE tbl (id LONG, part STRING) USING delta PARTITIONED BY (part)")
...
workload("ev_future_commit_info", "CommitInfo with unknown future fields",
    "evolvability") { w =>
  w.sql("CREATE TABLE tbl (id LONG) USING delta")
  w.sql("INSERT INTO tbl SELECT id FROM range(10)")
  val t = w.table("tbl")
...
workload("ev_missing_intermediate_version", "Log with version gap",
    "evolvability") { w =>
  w.sql("CREATE TABLE tbl (id LONG) USING delta")
...
workload("fc_unknown_field_in_add", "Forward compat - unknown field in add action",
    "formatCompat") { w =>
  w.sql("CREATE TABLE tbl (id LONG) USING delta")
  w.sql("INSERT INTO tbl SELECT id FROM range(10)")
  val t = w.table("tbl")
...
workload("fc_unknown_field_in_metadata", "Forward compat - unknown field in metadata",
    "formatCompat") { w =>
  w.sql("CREATE TABLE tbl (id LONG) USING delta")
  w.sql("INSERT INTO tbl SELECT id FROM range(10)")
  val t = w.table("tbl")
...
workload("fc_unknown_field_in_protocol", "Forward compat - unknown field in protocol",
    "formatCompat") { w =>
  w.sql("CREATE TABLE tbl (id LONG) USING delta")
  w.sql("INSERT INTO tbl SELECT id FROM range(10)")
  val t = w.table("tbl")
...
workload("fc_unknown_action_top_level", "Forward compat - unknown action at top level",
    "formatCompat") { w =>
  w.sql("CREATE TABLE tbl (id LONG) USING delta")
  w.sql("INSERT INTO tbl SELECT id FROM range(10)")
  val t = w.table("tbl")
...
workload("fc_null_fields_in_add", "Forward compat - null fields in add action",
    "formatCompat") { w =>
  w.sql("CREATE TABLE tbl (id LONG) USING delta")
  w.sql("INSERT INTO tbl SELECT id FROM range(10)")
  val t = w.table("tbl")
...
workload("fc_empty_json_line", "Forward compat - empty JSON line in commit file",
    "formatCompat") { w =>
  w.sql("CREATE TABLE tbl (id LONG) USING delta")
  w.sql("INSERT INTO tbl SELECT id FROM range(10)")
  val t = w.table("tbl")
...
workload("fc_extra_metadata_keys", "Forward compat - extra metadata configuration keys",
    "formatCompat") { w =>
  w.sql("CREATE TABLE tbl (id LONG) USING delta")
  w.sql("INSERT INTO tbl SELECT id FROM range(10)")
  val t = w.table("tbl")
