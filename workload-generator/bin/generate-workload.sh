...
# Find spark-shell: prefer Spark 4.1 with Scala 2.13, then SPARK_HOME, then PATH
SPARK_4_HOME="$HOME/spark/spark-4.1.0-bin-hadoop3"
if [[ -d "$SPARK_4_HOME" ]]; then
  SPARK_SHELL="$SPARK_4_HOME/bin/spark-shell"
else
...
if [[ -z "$SPARK_SHELL" || ! -x "$SPARK_SHELL" ]]; then
  ...
  echo "       Download Spark 4.1 from https://spark.apache.org/downloads.html"
  exit 1
fi

# Fetch Delta from Maven
SPARK_CONF=(
...
export WORKLOAD_PARALLEL="${PARALLEL}"
...
if [[ -d "$INPUT" ]]; then
  ...
  while IFS= read -r line; do
    SCRIPTS+=("$line")
  done < <(find "$INPUT" -name '*.scala' -type f | sort)
