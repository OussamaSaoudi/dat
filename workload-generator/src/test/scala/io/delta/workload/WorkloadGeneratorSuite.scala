...
class WorkloadGeneratorSuite extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("mutation: deleted data file produces error spec") {
    ...
    val errorMsg = spec.get("error").get("errorMessage").asText()
    ...
    assert(errorCode.contains("FILE_NOT_FOUND") || errorCode.contains("FileNotFoundException")
      || errorCode.contains("FILE_NOT_EXIST") || errorCode.contains("SparkException")
      || errorCode.contains("AnalysisException") || errorMsg.contains("does not exist"),
      s"Error should be file-related, got code: $errorCode, msg: $errorMsg")
  }
