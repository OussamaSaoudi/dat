  ...
  private[workload] def generateTable(
    ...
    try {
      ...
      try {
        ...
        if (ts.expectTableInfoFailure) {
          warnings += "Expected table_info.json to fail but it succeeded"
        }
      } catch {
        case e: Exception =>
          if (ts.expectTableInfoFailure) {
            // Write minimal table_info so the test output is valid
            val minimal = Map(
              "name" -> dirName,
              "description" -> ts.description,
              "error" -> s"Table metadata unavailable: ${e.getMessage}")
            JsonUtil.writeSpec(testOutputDir.resolve("table_info.json"), minimal)
          } else {
            throw e
          }
      }
...
class WorkloadContext private[workload] (
   ...
   * @param expectTableInfoFailure If true, allows table_info.json generation to fail
   *                               (e.g., for corrupted table tests).
   */
  def registerTable(name: String, expectTableInfoFailure: Boolean = false): TableHandle = {
    val path = resolveTablePath(name)
    val handle = new TableHandle(name, path, this)
    ensureTableSpec(handle, expectTableInfoFailure)
    handle
  }
...
trait WorkloadOps {
  ...
  /** Register a managed Spark table for spec capture. */
  def registerTable(name: String, expectTableInfoFailure: Boolean = false): TableHandle =
    current.registerTable(name, expectTableInfoFailure)
...
private[workload] class TableSpec(
  ...
  var expectTableInfoFailure: Boolean = false
}
